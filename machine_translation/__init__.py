...
from machine_translation.sampling import BleuTestor, BleuValidator, Sampler, SamplingBase
...
def main(mode, config, use_bokeh=False):
    ...
    if mode == "train":
        ...
        loader.set_model_parameters(model, loader.load_parameters(config['best_params']))
        ...
        bt = BleuTestor(sampling_input, samples=samples, config=config,
                        model=model, data_stream=test_stream, trg_ivocab=trg_ivocab,
                        normalize=config['normalized_bleu'])

        # print the final blue score on test set
        print(bt.do())
